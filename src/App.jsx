...
function isClosingText(s) {
  ...
  return ["sincerely,","regards,","best regards,","respectfully,","very truly yours,","yours truly,",
    "approved by:","approved by","noted by:","noted by","prepared by:","prepared by",
    "authorized by:","authorized by","submitted by:","submitted by","certified by:","certified by",
    "for approval:","for approval"].includes(t);
}
...
export default function SignDesk() {
  ...
  const onDocsUpload = (e) => {
    ...
    const newDocs = files.map(f => ({
      ...
      status: "pending", aiResult: null, pageInfos: null,
      signedBlob: null, open: false, previewRendered: false,
    }));
  ...
  const renderAndAnalyze = useCallback(async (doc) => {
    ...
    try {
      ...
      const imageContents = pageInfos.map(pi => ({
        type: "image",
        source: { type: "base64", media_type: "image/jpeg", data: pi.canvas.toDataURL("image/jpeg", 0.9).split(",")[1] },
      }));
      const pageMapNote = pageInfos.map((p, i) => `image index ${i} = page ${p.pageNum}`).join(", ");
      ...
      const res = await fetch("/api/analyze", {
        ...
        body: JSON.stringify({
          ...
          messages: [{ role: "user", content: [
            ...imageContents,
            { type: "text", text: `This document has ${pageInfos.length} page(s). Image mapping: ${pageMapNote}.\n\nFind ALL signature locations.\n\n${AI_PROMPT}` },
          ]}],
        }),
      });
      ...
      if (!aiResult?.found || !aiResult.locations?.length) {
        const fallback = findStrictLabelNamePlacement({ pageInfos });
        if (fallback) aiResult = fallback;
      }
      ...
      if (aiResult.found && aiResult.locations?.length) {
        aiResult.locations = aiResult.locations
          ...
          .filter(loc => loc.pageInfo && loc.width_percent > 0 && loc.height_percent > 0);
      }
      ...
      const fallback = findStrictLabelNamePlacement({ pageInfos: doc.pageInfos || [] });
      if (fallback) {
        updateDoc(doc.id, { aiResult: fallback, status: "pending" });
        showToast("AI failed, using text fallback.", "info");
        return;
      }
  ...
  const toggleCard = useCallback((doc) => {
    setDocs(prev => prev.map(d => d.id === doc.id ? { ...d, open: !d.open } : d));
  ...
  const handleSign = useCallback(async (doc) => {
    ...
    try {
      ...
      const embImg = mime.includes("jpeg") || mime.includes("jpg")
        ? await pdfDoc.embedJpg(imgBytes) : await pdfDoc.embedPng(imgBytes);
      ...
      if (finalResult?.found && finalResult.locations?.length) {
        for (const loc of finalResult.locations) {
          ...
          // loc.y_percent is canvas top-down (0=top of page)
          // PDF Y is bottom-up (0=bottom of page)
          // sigY = distance from bottom = height - (y_percent * height) - boxH
      ...
      if (placements.length === 0) throw new Error("No valid signature box found.");
  ...
  const toastColors = { info: G.accent2, success: G.green, error: G.red };
  const toastIcons  = { info: "◆", success: "✓", error: "✕" };
    ...
    <>
      ...
      <div style={{ minHeight:"100vh", background:G.bg, color:G.text, fontFamily:"'Outfit',sans-serif",
        ...
        <header style={{ position:"sticky", top:0, zIndex:100, display:"flex", alignItems:"center",
          justifyContent:"space-between", padding:"1rem 2rem", background:"rgba(11,12,16,0.85)",
          backdropFilter:"blur(18px)", borderBottom:`1px solid ${G.border}` }}>
          <div style={{ fontFamily:"'Cormorant Garamond',serif", fontSize:"1.6rem", fontWeight:700 }}>
            Sign<span style={{color:G.accent2}}>Desk</span>{" "}
            <span style={{fontSize:".6em",color:G.muted,fontFamily:"'Outfit',sans-serif",fontWeight:300}}>AI</span>
          </div>
          <div style={{ display:"flex", alignItems:"center", gap:".45rem", background:"rgba(124,106,247,0.15)",
            border:"1px solid rgba(124,106,247,0.3)", borderRadius:"100px", padding:".28rem .85rem",
            fontSize:".72rem", fontWeight:600, color:G.accent2, letterSpacing:".06em" }}>
            <span style={{ width:6,height:6,background:G.accent2,borderRadius:"50%",animation:"pulse 2s infinite",display:"inline-block" }}/>
        ...
        <main style={{ maxWidth:1060, margin:"0 auto", padding:"2.2rem 1.5rem 5rem" }}>
          <div style={{ marginBottom:"2rem" }}>
            <h1 style={{ fontFamily:"'Cormorant Garamond',serif", fontSize:"clamp(1.7rem,4vw,2.6rem)", fontWeight:700, lineHeight:1.1, marginBottom:".5rem" }}>
              Sign documents<br/>
              <span style={{ background:`linear-gradient(135deg,${G.accent2},${G.gold})`, WebkitBackgroundClip:"text", WebkitTextFillColor:"transparent" }}>intelligently.</span>
            </h1>
            <p style={{ color:G.muted2, fontSize:".92rem", maxWidth:540, lineHeight:1.65 }}>
              Upload your signature once. AI scans the PDF and stamps the signature exactly inside the detected box so the actual placement matches the shown location.
            </p>
          </div>
          ...
          <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:"1.1rem", marginBottom:"2rem" }}>
            <Panel label="Step 1" title="Your Signature">
              <DropZone icon="✍️" text={<>Upload your <strong>signature image</strong><br/>PNG with transparent background works best</>}>
                <input type="file" accept="image/*" onChange={onSigUpload} style={{ position:"absolute",inset:0,opacity:0,cursor:"pointer",width:"100%",height:"100%" }}/>
              </DropZone>
              {sig && (
                <div style={{ display:"flex",alignItems:"center",gap:".75rem",marginTop:".9rem",padding:".7rem",background:G.surface2,border:`1px solid ${G.border2}`,borderRadius:9 }}>
                  <img src={sig.dataUrl} alt="sig" style={{ maxHeight:48,maxWidth:150,objectFit:"contain",background:"rgba(255,255,255,.05)",borderRadius:5,padding:3 }}/>
                  <div style={{ flex:1,minWidth:0 }}>
                    <div style={{ fontSize:".83rem",fontWeight:600,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap" }}>{sig.name}</div>
                    <div style={{ fontSize:".71rem",color:G.green }}>✓ Ready to use</div>
                  </div>
                  <button onClick={() => setSig(null)} style={{ background:"none",border:"none",cursor:"pointer",color:G.muted,fontSize:".95rem",padding:3 }}>✕</button>
                </div>
              )}
            </Panel>
            <Panel label="Step 2" title="Upload Documents">
              <DropZone icon="📄" text={<>Upload <strong>PDF files</strong> here<br/>AI will locate signature fields automatically</>}>
                <input type="file" accept=".pdf" multiple onChange={onDocsUpload} style={{ position:"absolute",inset:0,opacity:0,cursor:"pointer",width:"100%",height:"100%" }}/>
              </DropZone>
            </Panel>
          </div>
          ...
          {docs.length > 0 && (
            <>
              <div style={{ display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:"1.1rem" }}>
                <h2 style={{ fontFamily:"'Cormorant Garamond',serif",fontSize:"1.35rem" }}>Documents to Review</h2>
                <span style={{ fontSize:".7rem",fontWeight:600,background:"rgba(124,106,247,.2)",color:G.accent2,border:"1px solid rgba(124,106,247,.3)",padding:".22rem .75rem",borderRadius:"100px" }}>
                  {docs.length} file{docs.length !== 1 ? "s" : ""}
                </span>
              </div>
              <div style={{ display:"flex",flexDirection:"column",gap:".9rem" }}>
                {docs.map(doc => (
                  <DocCard key={doc.id} doc={doc} onToggle={() => toggleCard(doc)} onSign={() => handleSign(doc)} onReject={() => handleReject(doc)} />
                ))}
              </div>
              <div style={{ display:"flex",alignItems:"center",gap:"2.2rem",flexWrap:"wrap",background:G.surface,border:`1px solid ${G.border}`,borderRadius:14,padding:"1.3rem 1.8rem",marginTop:"1.8rem" }}>
                {[{num:summary.total,lbl:"Total",color:G.text},{num:summary.signed,lbl:"Signed",color:G.green},{num:summary.rej,lbl:"Rejected",color:G.red},{num:summary.pend,lbl:"Pending",color:G.gold}].map(s => (
                  <div key={s.lbl} style={{ textAlign:"center" }}>
                    <span style={{ fontFamily:"'Cormorant Garamond',serif",fontSize:"2rem",fontWeight:700,display:"block",lineHeight:1,marginBottom:".15rem",color:s.color }}>{s.num}</span>
                    <span style={{ fontSize:".69rem",color:G.muted,textTransform:"uppercase",letterSpacing:".1em" }}>{s.lbl}</span>
                  </div>
                ))}
              </div>
            </>
      ...
      {toast && (
        <div style={{ position:"fixed",bottom:"1.6rem",right:"1.6rem",zIndex:999,background:G.surface2,border:`1px solid ${toastColors[toast.type]}55`,color:G.text,padding:".75rem 1.2rem",borderRadius:11,fontSize:".84rem",fontWeight:500,boxShadow:"0 10px 40px rgba(0,0,0,.5)",display:"flex",alignItems:"center",gap:".55rem",maxWidth:300 }}>
...
function Panel({ label, title, children }) {
  return (
    <div style={{ background:G.surface,border:`1px solid ${G.border}`,borderRadius:16,padding:"1.5rem" }}>
      <div style={{ fontSize:".67rem",fontWeight:700,letterSpacing:".16em",textTransform:"uppercase",color:G.accent2,marginBottom:".4rem" }}>{label}</div>
      <h2 style={{ fontFamily:"'Cormorant Garamond',serif",fontSize:"1.15rem",fontWeight:600,marginBottom:".9rem" }}>{title}</h2>
      {children}
    </div>
  );
}
...
function DropZone({ icon, text, children }) {
  ...
  return (
    <div onDragOver={e=>{e.preventDefault();setOver(true)}} onDragLeave={()=>setOver(false)} onDrop={e=>{e.preventDefault();setOver(false);}}
      style={{ border:`1.5px dashed rgba(124,106,247,${over?".7":".35"})`,borderRadius:11,padding:"1.6rem 1rem",textAlign:"center",cursor:"pointer",position:"relative",background:over?"rgba(124,106,247,.07)":"transparent",transition:"all .2s" }}>
      {children}
      <span style={{ fontSize:"1.9rem",display:"block",marginBottom:".4rem" }}>{icon}</span>
...
function DocCard({ doc, onToggle, onSign, onReject }) {
  ...
  useEffect(() => {
    ...
    doc.pageInfos.forEach(pi => {
      ...
      if (doc.aiResult?.found && doc.aiResult.locations?.length) {
        doc.aiResult.locations.filter(loc => loc.pageInfo?.pageNum === pi.pageNum).forEach(loc => {
          ...
          ov.style.cssText = `position:absolute;border:2px solid ${G.green};border-radius:4px;
            ...
            width:${loc.width_percent * pi.width}px;height:${Math.max(loc.height_percent * pi.height,18)}px;`;
          const lbl = document.createElement("div");
          lbl.style.cssText = `position:absolute;top:-19px;left:0;font-size:9px;background:${G.green};color:#000;padding:1px 5px;border-radius:3px;font-weight:700;white-space:nowrap;font-family:'Outfit',sans-serif;`;
  ...
  return (
    <div style={{ background:G.surface, border:`1px solid ${doc.status==="signed"?G.green:doc.status==="rejected"?G.red:G.border}`,
      ...
      <div onClick={onToggle} style={{ display:"flex",alignItems:"center",padding:".9rem 1.1rem",gap:".85rem",cursor:"pointer",userSelect:"none" }}>
        <div style={{ width:36,height:36,background:G.surface2,borderRadius:8,display:"flex",alignItems:"center",justifyContent:"center",fontSize:"1.1rem",flexShrink:0 }}>📄</div>
        <div style={{ flex:1,minWidth:0 }}>
          <div style={{ fontSize:".9rem",fontWeight:600,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap" }}>{doc.name}</div>
          <div style={{ fontSize:".73rem",color:G.muted }}>{fmt(doc.size)}</div>
        </div>
        <span style={{ fontSize:".67rem",fontWeight:700,letterSpacing:".09em",textTransform:"uppercase",padding:".2rem .65rem",borderRadius:"100px",flexShrink:0,background:statusInfo.bg,color:statusInfo.color }}>{statusInfo.label}</span>
        <span style={{ color:G.muted,fontSize:".8rem",marginLeft:".35rem",transition:"transform .25s",display:"inline-block",transform:doc.open?"rotate(180deg)":"" }}>▾</span>
      </div>

      {doc.open && (
        <div style={{ padding:"0 1.1rem 1.1rem",borderTop:`1px solid ${G.border}` }}>
          <div style={{ margin:"1rem 0",padding:".8rem 1rem",background:"rgba(124,106,247,.09)",border:"1px solid rgba(124,106,247,.22)",borderRadius:9,fontSize:".81rem",lineHeight:1.55,color:G.muted2 }}>
            {!doc.aiResult && doc.status === "analyzing" && <><strong style={{color:G.accent2}}>🤖 AI Analysis</strong> — Scanning pages for signature fields…</>}
            {!doc.aiResult && doc.status !== "analyzing" && <><strong style={{color:G.accent2}}>🤖 AI Analysis</strong> — Opening document…</>}
            {doc.aiResult?.found && <>
              <strong style={{color:G.accent2}}>🤖 AI found {doc.aiResult.locations.length} signature location{doc.aiResult.locations.length>1?"s":""}</strong>
              {doc.aiResult.locations.map((l,i) => (
                <div key={i} style={{display:"flex",alignItems:"flex-start",gap:".45rem",marginTop:".4rem"}}>
                  <span style={{color:G.accent,fontSize:".5rem",marginTop:".25rem"}}>◆</span>{l.description}
                </div>
              ))}
              <div style={{fontSize:".76rem",color:G.muted,marginTop:".35rem"}}>{doc.aiResult.reasoning}</div>
            </>}
            {doc.aiResult && !doc.aiResult.found && <>
              <strong style={{color:G.accent2}}>🤖 AI Analysis</strong> — No explicit field found.{" "}
              <span style={{color:G.gold}}>Will try strict label-name fallback only.</span>
              {doc.aiResult.reasoning && <div style={{fontSize:".76rem",color:G.muted,marginTop:".3rem"}}>{doc.aiResult.reasoning}</div>}
            </>}
          </div>

          <div style={{ background:"#111320",borderRadius:9,overflow:"hidden",marginBottom:".9rem",maxHeight:440,overflowY:"auto",display:"flex",flexDirection:"column",alignItems:"center",gap:4,padding:7 }}>
            {doc.status === "analyzing" && !doc.pageInfos && (
              <div style={{padding:"2.5rem",textAlign:"center",color:G.muted,fontSize:".83rem"}}>
                <div style={{display:"inline-block",width:22,height:22,border:`2px solid ${G.border2}`,borderTopColor:G.accent,borderRadius:"50%",animation:"spin .75s linear infinite",marginBottom:".6rem"}}/>
                <br/>Rendering &amp; analyzing…
              </div>
            )}
            {doc.status === "signed" && <div style={{padding:"2rem",textAlign:"center",color:G.green,fontSize:".9rem"}}>✓ Document signed. Download below.</div>}
          ...
          <div style={{ display:"flex",gap:".65rem",flexWrap:"wrap" }}>
            <button onClick={onSign} disabled={!isPending}
              style={{ flex:1,minWidth:110,display:"flex",alignItems:"center",justifyContent:"center",gap:".4rem",padding:".7rem .9rem",borderRadius:9,fontFamily:"'Outfit',sans-serif",fontSize:".86rem",fontWeight:600,cursor:isPending?"pointer":"not-allowed",border:"none",opacity:isPending?1:.35,background:"linear-gradient(135deg,#5b4fcf,#7c6af7)",color:"#fff",boxShadow:"0 4px 18px rgba(124,106,247,.35)" }}>
              {doc.status === "signing" ? "…Signing" : "✦ Sign Document"}
            </button>
            <button onClick={onReject} disabled={!isPending}
              style={{ flex:1,minWidth:110,display:"flex",alignItems:"center",justifyContent:"center",gap:".4rem",padding:".7rem .9rem",borderRadius:9,fontFamily:"'Outfit',sans-serif",fontSize:".86rem",fontWeight:600,cursor:isPending?"pointer":"not-allowed",border:`1px solid rgba(248,113,113,.3)`,background:G.surface2,color:G.red,opacity:isPending?1:.35 }}>
              {doc.status === "rejected" ? "✕ Rejected" : "✕ Reject"}
            </button>
            {doc.signedBlob && (
              <a href={URL.createObjectURL(doc.signedBlob)} download={doc.name.replace(".pdf","") + "_signed.pdf"}
                style={{ flex:1,minWidth:110,display:"flex",alignItems:"center",justifyContent:"center",gap:".4rem",padding:".7rem .9rem",borderRadius:9,fontFamily:"'Outfit',sans-serif",fontSize:".86rem",fontWeight:600,background:"linear-gradient(135deg,#1a7a55,#34d399)",color:"#fff",textDecoration:"none",boxShadow:"0 4px 18px rgba(52,211,153,.25)" }}>
                ⬇ Download Signed
              </a>
            )}
          </div>
