...
function useScript(src) {
  ...
  useEffect(() => {
    ...
    s.onerror = () => {
      // Retry with unpkg fallback
      const fallback = src
        .replace("cdnjs.cloudflare.com/ajax/libs/pdf.js/3.11.174", "unpkg.com/pdfjs-dist@3.11.174/build")
        .replace("cdnjs.cloudflare.com/ajax/libs/pdf-lib/1.17.1", "unpkg.com/pdf-lib@1.17.1/dist");
      if (fallback !== src) {
    ...
    try {
      ...
      if (doc.aiResult?.found && doc.aiResult.locations?.length) {
        for (const loc of doc.aiResult.locations) {
          ...
          // Place signature: PDF Y = height - (canvas top of box) - box height
          const canvasTop = loc.y_percent * height;
          const boxH      = loc.height_percent * height;
          const sigY      = height - canvasTop - boxH;
      ...
      showToast(`"${doc.name}" signed at ${placements.length} location${placements.length > 1 ? "s" : ""}!`, "success");
    ...
    <>
      ...
      <div style={{ minHeight:"100vh", background:G.bg, color:G.text, fontFamily:"'Outfit',sans-serif",
        ...
        <header style={{ position:"sticky", top:0, zIndex:100, display:"flex", alignItems:"center",
          ...
          <div style={{ display:"flex", alignItems:"center", gap:".45rem",
            ...
            borderRadius:"100px", padding:".28rem .85rem",
        ...
        <main style={{ maxWidth:1060, margin:"0 auto", padding:"2.2rem 1.5rem 5rem" }}>
          ...
          <div style={{ marginBottom:"2rem" }}>
            <h1 style={{ fontFamily:"'Cormorant Garamond',serif", fontSize:"clamp(1.7rem,4vw,2.6rem)",
              ...
              Sign documents<br/>
              <span style={{ background:`linear-gradient(135deg,${G.accent2},${G.gold})`,
          ...
          <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:"1.1rem", marginBottom:"2rem" }}>
            <Panel label="Step 1" title="Your Signature">
              ...
              {sig && (
                <div style={{ display:"flex",alignItems:"center",gap:".75rem",marginTop:".9rem",
                  padding:".7rem",background:G.surface2,border:`1px solid ${G.border2}`,borderRadius:9 }}>
                  <img src={sig.dataUrl} alt="sig" style={{ maxHeight:48,maxWidth:150,objectFit:"contain",
                    background:"rgba(255,255,255,.05)",borderRadius:5,padding:3 }}/>
                  ...
                  <button onClick={() => setSig(null)}
                    style={{ background:"none",border:"none",cursor:"pointer",color:G.muted,fontSize:".95rem",padding:3 }}>✕</button>
                </div>
              )}
            </Panel>
          ...
          {docs.length > 0 && (
            <>
              <div style={{ display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:"1.1rem" }}>
                ...
                <span style={{ fontSize:".7rem",fontWeight:600,background:"rgba(124,106,247,.2)",
                  color:G.accent2,border:"1px solid rgba(124,106,247,.3)",padding:".22rem .75rem",borderRadius:"100px" }}>
                ...
                {[
                  ...
                  <div key={s.lbl} style={{ textAlign:"center" }}>
                    <span style={{ fontFamily:"'Cormorant Garamond',serif",fontSize:"2rem",fontWeight:700,
                      display:"block",lineHeight:1,marginBottom:".15rem",color:s.color }}>{s.num}</span>
      ...
      {toast && (
        <div style={{ position:"fixed",bottom:"1.6rem",right:"1.6rem",zIndex:999,
          ...
          padding:".75rem 1.2rem",borderRadius:11,fontSize:".84rem",fontWeight:500,
          boxShadow:"0 10px 40px rgba(0,0,0,.5)",
          display:"flex",alignItems:"center",gap:".55rem",maxWidth:300 }}>
...
function DropZone({ icon, text, children }) {
  ...
  return (
    <div onDragOver={e=>{e.preventDefault();setOver(true)}} onDragLeave={()=>setOver(false)} onDrop={e=>{e.preventDefault();setOver(false);}}
      style={{ border:`1.5px dashed rgba(124,106,247,${over?".7":".35"})`,borderRadius:11,
        padding:"1.6rem 1rem",textAlign:"center",cursor:"pointer",position:"relative",
        background:over?"rgba(124,106,247,.07)":"transparent",transition:"all .2s" }}>
      {children}
  ...
  return (
    <div style={{ background:G.surface,
      ...
      <div onClick={onToggle} style={{ display:"flex",alignItems:"center",padding:".9rem 1.1rem",
        ...
        <span style={{ fontSize:".67rem",fontWeight:700,letterSpacing:".09em",textTransform:"uppercase",
          padding:".2rem .65rem",borderRadius:"100px",flexShrink:0,
          background:statusInfo.bg,color:statusInfo.color }}>{statusInfo.label}</span>
        <span style={{ color:G.muted,fontSize:".8rem",marginLeft:".35rem",
        ...
        <div style={{ padding:"0 1.1rem 1.1rem",borderTop:`1px solid ${G.border}` }}>
          ...
          <div style={{ margin:"1rem 0",padding:".8rem 1rem",
            ...
            {doc.aiResult && !doc.aiResult.found && <>
              ...
              <span style={{color:G.gold}}>Will place at default bottom-right.</span>
          ...
          <div style={{ background:"#111320",borderRadius:9,overflow:"hidden",marginBottom:".9rem",
            ...
            {doc.status === "signed" && (
              <div style={{padding:"2rem",textAlign:"center",color:G.green,fontSize:".9rem"}}>
          ...
          <div style={{ display:"flex",gap:".65rem",flexWrap:"wrap" }}>
            <button onClick={onSign} disabled={!isPending}
              style={{ flex:1,minWidth:110,display:"flex",alignItems:"center",justifyContent:"center",gap:".4rem",
                padding:".7rem .9rem",borderRadius:9,fontFamily:"'Outfit',sans-serif",fontSize:".86rem",fontWeight:600,
                cursor:isPending?"pointer":"not-allowed",border:"none",opacity:isPending?1:.35,
                background:"linear-gradient(135deg,#5b4fcf,#7c6af7)",color:"#fff",
            ...
            <button onClick={onReject} disabled={!isPending}
              style={{ flex:1,minWidth:110,display:"flex",alignItems:"center",justifyContent:"center",gap:".4rem",
                padding:".7rem .9rem",borderRadius:9,fontFamily:"'Outfit',sans-serif",fontSize:".86rem",fontWeight:600,
                cursor:isPending?"pointer":"not-allowed",border:`1px solid rgba(248,113,113,.3)`,
                background:G.surface2,color:G.red,opacity:isPending?1:.35 }}>
              {doc.status === "rejected" ? "✕ Rejected" : "✕ Reject"}
            </button>
            {doc.signedBlob && (
              <a href={URL.createObjectURL(doc.signedBlob)}
                download={doc.name.replace(".pdf","") + "_signed.pdf"}
                style={{ flex:1,minWidth:110,display:"flex",alignItems:"center",justifyContent:"center",gap:".4rem",
                  padding:".7rem .9rem",borderRadius:9,fontFamily:"'Outfit',sans-serif",fontSize:".86rem",fontWeight:600,
                  background:"linear-gradient(135deg,#1a7a55,#34d399)",color:"#fff",textDecoration:"none",
